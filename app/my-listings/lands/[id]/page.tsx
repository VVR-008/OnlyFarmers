...
export default function EditLandPage() {
  ...
  const fetchLandData = async () => {
    try {
      const response = await fetch(`/api/listings/lands?id=${landId}`, {
        headers: {
          "x-user-id": user?.id || "",
        },
      });
      ...
      const land = data.listing;

      // Check if user owns this listing
      if (land.seller !== user?.id) {
        router.push('/my-listings');
        return;
      }

      // Parse location
      const locationParts = land.location.address?.split(", ") || [];
      ...
      setFormData({
        ...
        areaUnit: land.area?.unit || "acres",
        priceTotal: land.price?.total?.toString() || "",
        soilType: land.soilType || "black",
        irrigation: land.irrigation || "borewell",
        ...
        locationDistrict: district || land.location.district || "",
        locationState: state || land.location.state || "",
      });

      // Set image types (assume all are URLs for existing listings)
      setImageTypes(land.images?.map(() => "url") || [null, null, null]);
    ...
    try {
      ...
      const response = await fetch(`/api/listings/lands?id=${landId}`, {
        ...
        body: JSON.stringify({
          ...
          price: {
            total: Number(formData.priceTotal),
            perAcre: Number(formData.priceTotal) / Number(formData.area),
          },
      ...
      if (response.ok) {
        setSuccess(true);
        setTimeout(() => {
          router.push('/my-listings?type=lands');
        }, 2000);
      } else {
          ...
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block font-medium mb-1">Land Type*</label>
                  <select
              ...
              <div>
                <label className="block font-medium mb-1">Irrigation*</label>
                <select
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="rounded"
                  />
                  <label htmlFor="roadAccess" className="text-sm">Road Access</label>
                </div>
                <div className="flex items-center gap-2">
                  <input
                    type="checkbox"
                    ...
                    className="rounded"
                  />
                  <label htmlFor="electricityConnection" className="text-sm">Electricity Connection</label>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  required
                  placeholder="Provide detailed information about your land"
                />
              </div>
            </div>
          </div>
